...
MemID HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::allocate(size_t p_size, size_t p_alignment) {
...
MemID HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::check_posible_free(size_t p_aligned_size, size_t p_alignment) {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
...
template <bool CoalesceEnabled>
MemID HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::find_valid_chunk(size_t p_aligned_size, size_t p_alignment) {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
...
void HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::deallocate(MemID p_mem) {
    WBE_DEBUG_ASSERT(!(mutex.is_unique_locked_by_current_thread()));
    if (p_mem == MEM_NULL) {
        return;
    }
...
char* HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::get_next_free_memory(char* p_from) {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
...
void* HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::acquire_memory(char* p_idle_chunk, char* p_mem_start, size_t p_mem_size) {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
...
void HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::insert_free_memory(char* p_insert_start, size_t p_insert_size) {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
    WBE_HAAAPIL_SET_CHUNK_HEADER(p_insert_start, HeaderType::IDLE, p_insert_size);
    coalesce_chunk(p_insert_start);
}

size_t HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::get_remain_size() const {
    char* curr = mem_chunk;
    size_t total = 0;
...
bool HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::is_in_pool(MemID p_mem_id) const {
    boost::shared_lock lock(mutex);
    return unguarded_is_in_pool(p_mem_id);
}

void HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::unguarded_check_broken() const {
    char* curr = mem_chunk;
...
void HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::coalesce_all() const {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
    char* curr = mem_chunk;
    do {
        coalesce_chunk(curr);
...
void HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::coalesce_chunk(char* p_chunk) const {
    WBE_DEBUG_ASSERT(mutex.is_unique_locked_by_current_thread());
...
HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::operator std::string() const {
    boost::shared_lock lock(mutex);
    return unguarded_to_string();
}

bool HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::unguarded_is_in_pool(MemID p_mem_id) const {
    char* curr = mem_chunk;
...
std::string HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList::unguarded_to_string() const {
    std::stringstream ss;
    ss << "{";
    ss << R"("type":"HeapAllocatorAtomicSharedMutexAlignedPoolImplicitList",)";
    ss << "\"total_size\":" << size << ",";
    ss << "\"chunk_layout\":[";
    char* curr = mem_chunk;
    bool first = true;
